...
import { cleanupExpiredClassroom, listAllClasses, listMyClasses, syncClassroomDirectories } from "../firebase/classroomRepository.js";
...
import { isClassroomExpired } from "../utils/classroomExpiration.js";
...
function renderAllClasses(classes){allClassList.innerHTML=classes.length?classes.map(c=>{const active=c.status==='active'&&!isClassroomExpired(c);return `<article class="all-class-row"><div class="all-class-info"><h3>${escapeHtml(c.title)}</h3><p>${escapeHtml(c.className||'未設定班級')}・${c.studentCount||0} 位學生</p></div><span class="badge ${active?'online':'disabled'}">${active?'進行中':'停用中'}</span></article>`;}).join(""):'<div class="empty">系統目前沒有課程。</div>';}
async function load(){renderStats();try{user ||= await ensureAnonymousUser();}catch(e){const message=`<div class="notice">${escapeHtml(explainError(e))}</div>`;list.innerHTML=message;allClassList.innerHTML=message;return;}let active=[];try{const classes=await listMyClasses(user.uid);rememberVisibleClassrooms(classes);for(const classroom of classes){try{if(await cleanupExpiredClassroom(classroom.id,user.uid,classroom))continue;}catch(error){console.error(error);}active.push(classroom);}renderStats();list.innerHTML=active.length?active.map(c=>`<article class="class-card"><span class="badge ${c.status==='active'?'online':''}">${c.status==='active'?'進行中':'已結束'}</span><h3>${escapeHtml(c.title)}</h3><p>${escapeHtml(c.className||'未設定班級')}・${c.studentCount||0} 位學生</p><div class="card-actions"><a class="button-link" href="./teacher.html?class=${c.id}">課堂設定</a><a class="button-link" href="./monitor.html?class=${c.id}">多格監看</a></div></article>`).join(""):'<div class="empty">這個瀏覽器尚未建立課堂。</div>';}catch(e){list.innerHTML=`<div class="notice">${escapeHtml(explainError(e))}</div>`;}try{await syncClassroomDirectories(active);}catch(e){console.error("系統課程清單補登失敗：",e);}try{renderAllClasses(await listAllClasses());}catch(e){allClassList.innerHTML=`<div class="notice">${escapeHtml(explainError(e))}</div>`;}}
