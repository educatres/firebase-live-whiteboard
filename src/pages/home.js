...
async function load(){try{user ||= await ensureAnonymousUser();const classes=await listMyClasses(user.uid),active=[];for(const classroom of classes){try{if(await cleanupExpiredClassroom(classroom.id,user.uid,classroom))continue;}catch(error){console.error(error);}active.push(classroom);}list.innerHTML=active.length?active.map(c=>`<article class="class-card"><span class="badge ${c.status==='active'?'online':''}">${c.status==='active'?'進行中':'已結束'}</span><h3>${escapeHtml(c.title)}</h3><p>${escapeHtml(c.className||'未設定班級')}・${c.studentCount||0} 位學生</p><div class="card-actions"><a class="button-link" href="./teacher.html?class=${c.id}">課堂設定</a><a class="button-link" href="./monitor.html?class=${c.id}">多格監看</a></div></article>`).join(""):'<div class="empty">這個瀏覽器尚未建立課堂。</div>';}catch(e){list.innerHTML=`<div class="notice">${escapeHtml(explainError(e))}</div>`;}}
